...
| `filter_engine/` **(live, v2.5 Step 9)** | A fully modular, self-registering plugin system for filters — 39 built-in (12 data-backed, 27 honestly dormant), AND/OR/NOT/nested composition, validation/statistics/history | Any platform-specific logic; any comparison formula another module already owns (reuses `search.criteria`'s existing definitions and the Analysis Engine's stored scores rather than duplicating either) | [25_Dynamic_Filter_Engine.md](25_Dynamic_Filter_Engine.md) |
| `geography/` **(live, v2.5 Step 10)** | A provider-independent Geographic Intelligence Engine — distance/travel-time calculators, nearby search (17 categories), caching, plugin providers (`haversine` built in) — enriches apartments with `GeoEnrichment` without ever mutating them | Any specific map/routing vendor's SDK (hardcoded provider selection); ranking/filtering business logic itself | [26_Geographic_Intelligence.md](26_Geographic_Intelligence.md) |
